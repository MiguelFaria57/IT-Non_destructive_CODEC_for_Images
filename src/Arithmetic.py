...
def main():
    ...
    freqs = get_frequencies(imagem)
    freqs.increment(256)

    with open(imagem, "rb") as inp, \
            contextlib.closing(BitOutputStream(open("compressedArithmetic.dat", "wb"))) as bitout:
        write_frequencies(bitout, freqs)
        compress(freqs, inp, bitout)

    size = os.path.getsize('compressedArithmetic.dat')
